...
def kru2uni(kru_text: str) -> str:
    """Convert KrutiDev text (str) to Unicode Hindi (str)."""
    ...
    # Fix misplaced – —
    misplaced_dash = re.compile(r'[\u2014\u2013]')
    for m in misplaced_dash.finditer(kru_text):
        index = m.start()
        if (
            index < len(kru_text) - 1
            and kru_text[index + 1]
            not in krutidev_consonants + krutidev_unattached_vowel_signs
        ):
            kru_text = kru_text[:index] + '&' + kru_text[index + 1:]

    # Apply direct mappings
    for src, tgt in k2u:
        kru_text = kru_text.replace(src, tgt)

    kru_text = kru_text.replace('\xb1', 'Z\u0902')
    kru_text = kru_text.replace('\xc6', '\u0930\u094df')

    # f + consonant → consonant + ि
    while re.search(r'f(.)', kru_text):
        kru_text = re.sub(r'f(.)', lambda m: m.group(1) + '\u093f', kru_text)

    kru_text = kru_text.replace('\xc7', 'fa')
    kru_text = kru_text.replace('\xaf', 'fa')
    kru_text = kru_text.replace('\xc9', '\u0930\u094dfa')

    # fa + consonant → consonant + िं
    while re.search(r'fa(.)', kru_text):
        kru_text = re.sub(r'fa(.)', lambda m: m.group(1) + '\u093f\u0902', kru_text)

    kru_text = kru_text.replace('\xca', '\u0940Z')

    # ि् + consonant reordering
    while re.search('\u093f\u094d(.)', kru_text):
        kru_text = re.sub('\u093f\u094d(.)', lambda m: '\u094d' + m.group(1) + '\u093f', kru_text)

    kru_text = kru_text.replace('\u094dZ', 'Z')

    # Reposition reph (Z)
    while re.search(r'(.)Z', kru_text):
        m = re.search(r'(.)Z', kru_text)
        ch = m.group(1)
        idx = kru_text.index(ch + 'Z')
        while idx > 0 and kru_text[idx - 1] in unicode_vowel_signs:
            idx -= 1
            ch = kru_text[idx] + ch
        kru_text = kru_text.replace(ch + 'Z', '\u0930\u094d' + ch, 1)

    # Illegal matra positions
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KrutiDev → Unicode Hindi Converter (Python 3)")
    parser.add_argument(
        "-i", "--input",
    ...
    for line in args.input:
        args.output.write(kru2uni(line))